...
<div class="gl-countdown-overlay" id="gl-countdown-overlay" style="display:none">
    ...
    `}bindOverlayEvents(e){let t=this.overlayShadow;if(t){if(t.getElementById("gl-close")?.addEventListener("click",()=>this.closeOverlay()),t.getElementById("gl-overlay")?.addEventListener("click",r=>{r.target.id==="gl-overlay"&&this.closeOverlay()}),e==="upload"){let r=t.getElementById("gl-generate-btn"),o=l=>{this.selectedFile=l,r&&(r.disabled=!1)},n=t.getElementById("gl-tab-upload"),i=t.getElementById("gl-tab-camera"),a=t.getElementById("gl-panel-upload"),d=t.getElementById("gl-panel-camera");n?.addEventListener("click",()=>{this.stopCamera(),n.classList.add("gl-tab-active"),i?.classList.remove("gl-tab-active"),a&&(a.style.display="block"),d&&(d.style.display="none")}),i?.addEventListener("click",async()=>{i.classList.add("gl-tab-active"),n?.classList.remove("gl-tab-active"),a&&(a.style.display="none"),d&&(d.style.display="block"),await this.startCamera(t)});let s=t.getElementById("gl-upload-zone"),p=t.getElementById("gl-file-input"),u=t.getElementById("gl-upload-preview-wrap"),c=t.getElementById("gl-upload-preview-img"),g=l=>{if(!l.type.startsWith("image/"))return;o(l);let m=URL.createObjectURL(l);c&&(c.src=m),s&&(s.style.display="none"),u&&(u.style.display="block")};s?.addEventListener("click",()=>p?.click()),t.getElementById("gl-change-photo")?.addEventListener("click",()=>{this.selectedFile=null,r&&(r.disabled=!0),s&&(s.style.display="block"),u&&(u.style.display="none"),p?.click()}),p?.addEventListener("change",()=>{let l=p.files?.[0];l&&g(l)}),s?.addEventListener("dragover",l=>{l.preventDefault(),s.classList.add("gl-drag-over")}),s?.addEventListener("dragleave",()=>s.classList.remove("gl-drag-over")),s?.addEventListener("drop",l=>{l.preventDefault(),s.classList.remove("gl-drag-over");let m=l.dataTransfer?.files[0];m&&g(m)}),t.getElementById("gl-capture-now")?.addEventListener("click",()=>{this.capturePhoto(t,o)}),t.getElementById("gl-timer-btn")?.addEventListener("click",()=>{this.startCountdown(t,5,o)}),t.getElementById("gl-retake-btn")?.addEventListener("click",async()=>{this.selectedFile=null,r&&(r.disabled=!0);let l=t.getElementById("gl-camera-preview-wrap"),m=t.getElementById("gl-camera-wrap"),f=t.getElementById("gl-camera-controls"),y=t.getElementById("gl-capture-now"),v=t.getElementById("gl-timer-btn"),x=t.getElementById("gl-lighting-bar");l&&(l.style.display="none"),m&&(m.style.display="block"),f&&(f.style.display="flex"),x&&(x.style.display="flex"),y&&(y.disabled=!0),v&&(v.disabled=!0),await this.startCamera(t)}),r?.addEventListener("click",async()=>{this.selectedFile&&await this.generateTryOn(this.selectedFile)}),t.getElementById("gl-model-primary")?.addEventListener("click",()=>{this.selectedProvider="primary",t.getElementById("gl-model-primary")?.classList.add("gl-model-active"),t.getElementById("gl-model-fallback")?.classList.remove("gl-model-active")}),t.getElementById("gl-model-fallback")?.addEventListener("click",()=>{this.selectedProvider="fallback",t.getElementById("gl-model-fallback")?.classList.add("gl-model-active"),t.getElementById("gl-model-primary")?.classList.remove("gl-model-active")}),t.getElementById("gl-buy-btn-upload")?.addEventListener("click",()=>{let l=this.currentProduct?.id;this.trackEvent("buy_clicked",{product_id:l}),l&&window.glAddToCart?window.glAddToCart(l):(document.querySelector('button[name="add"]')||document.querySelector('[class*="add-to-cart"]'))?.click(),this.closeOverlay()}),t.getElementById("gl-wl-btn-upload")?.addEventListener("click",()=>{let l=this.currentProduct?.id;if(this.trackEvent("wishlist_clicked",{product_id:l}),l&&window.glAddToWL){window.glAddToWL(l);let m=t.getElementById("gl-wl-btn-upload");m&&(m.textContent="\u2665 Wishlisted")}})}e==="result"&&(t.getElementById("gl-retry")?.addEventListener("click",()=>{this.selectedFile=null,this.renderOverlay("upload")}),t.getElementById("gl-buy-btn")?.addEventListener("click",()=>{let r=this.currentProduct?.id;this.trackEvent("buy_clicked",{product_id:r}),r&&window.glAddToCart?window.glAddToCart(r):(document.querySelector('button[name="add"]')||document.querySelector('[class*="add-to-cart"]'))?.click(),this.closeOverlay()}),t.getElementById("gl-wl-btn")?.addEventListener("click",()=>{let r=this.currentProduct?.id;if(this.trackEvent("wishlist_clicked",{product_id:r}),r&&window.glAddToWL){window.glAddToWL(r);let o=t.getElementById("gl-wl-btn");o&&(o.textContent="\u2665 Wishlisted")}})),e==="error"&&t.getElementById("gl-retry")?.addEventListener("click",()=>{this.selectedFile=null,this.renderOverlay("upload")})}}async startCamera(e){let t=e.getElementById("gl-camera-video"),r=e.getElementById("gl-camera-error"),o=e.getElementById("gl-capture-now"),n=e.getElementById("gl-timer-btn"),i=e.getElementById("gl-camera-wrap");this.stopCamera();try{let a=await navigator.mediaDevices.getUserMedia({video:{facingMode:"user",width:{ideal:640},height:{ideal:480}}});this.cameraStream=a,t&&(t.srcObject=a),i&&(i.style.display="block"),r&&(r.style.display="none"),this.monitorLighting(e)}catch{r&&(r.style.display="block"),i&&(i.style.display="none"),o&&(o.disabled=!0),n&&(n.disabled=!0)}}capturePhoto(e,t){let r=e.getElementById("gl-camera-video"),o=e.getElementById("gl-camera-canvas");if(!r||!o||!r.videoWidth)return;o.width=r.videoWidth,o.height=r.videoHeight;let n=o.getContext("2d");if(n){if(this.lastBrightness>0&&this.lastBrightness<80){let i=Math.min(2,100/Math.max(this.lastBrightness,20));n.filter=`brightness(${i.toFixed(2)})`}n.drawImage(r,0,0),n.filter="none",this.stopCamera(),o.toBlob(i=>{if(!i)return;let a=new File([i],"selfie.jpg",{type:"image/jpeg"}),d=e.getElementById("gl-camera-wrap"),s=e.getElementById("gl-camera-controls"),p=e.getElementById("gl-lighting-bar"),u=e.getElementById("gl-camera-preview-wrap"),c=e.getElementById("gl-camera-preview-img"),g=URL.createObjectURL(a);c&&(c.src=g),d&&(d.style.display="none"),s&&(s.style.display="none"),p&&(p.style.display="none"),u&&(u.style.display="block"),t(a)},"image/jpeg",.92)}}monitorLighting(e){this.lightingInterval&&(clearInterval(this.lightingInterval),this.lightingInterval=null);let t=e.getElementById("gl-camera-video"),r=e.getElementById("gl-lighting-dot"),o=e.getElementById("gl-lighting-text"),n=e.getElementById("gl-capture-now"),i=e.getElementById("gl-timer-btn"),a=document.createElement("canvas");a.width=64,a.height=48;let d=a.getContext("2d");this.lightingInterval=window.setInterval(()=>{if(!d||!t||!t.videoWidth)return;d.drawImage(t,0,0,64,48);let s=d.getImageData(0,0,64,48).data,p=0;for(let l=0;l<s.length;l+=4)p+=.299*s[l]+.587*s[l+1]+.114*s[l+2];let u=p/(64*48);this.lastBrightness=u;let c=u>=80,g=u>=50;r&&(r.style.background=c?"#22c55e":g?"#f59e0b":"#ef4444"),o&&(o.textContent=c?"\u2713 Good lighting":g?"\u26A0 Brighter light will improve results":"\u2717 Too dark \u2014 move to better lighting"),this.countdownTimer||(n&&(n.disabled=!g),i&&(i.disabled=!g))},500)}startCountdown(e,t,r){let o=e.getElementById("gl-countdown-overlay"),n=e.getElementById("gl-countdown-num"),i=e.getElementById("gl-capture-now"),a=e.getElementById("gl-timer-btn");this.countdownTimer&&(clearInterval(this.countdownTimer),this.countdownTimer=null),o&&(o.style.display="flex"),i&&(i.disabled=!0),a&&(a.disabled=!0);let d=t;n&&(n.textContent=String(d)),this.countdownTimer=window.setInterval(()=>{d--,n&&(n.textContent=String(d)),d<=0&&(clearInterval(this.countdownTimer),this.countdownTimer=null,o&&(o.style.display="none"),i&&(i.disabled=!1),a&&(a.disabled=!1),this.capturePhoto(e,r))},1e3)}stopCamera(){this.countdownTimer&&(clearInterval(this.countdownTimer),this.countdownTimer=null),this.lightingInterval&&(clearInterval(this.lightingInterval),this.lightingInterval=null),this.cameraStream&&(this.cameraStream.getTracks().forEach(e=>e.stop()),this.cameraStream=null)}closeOverlay(){this.stopCamera(),this.overlayShadow?.getElementById("gl-overlay")?.remove(),this.trackEvent("tryon_closed",{product_id:this.currentProduct?.id})}async generateTryOn(e){this.stopCamera(),this.renderOverlay("processing"),this.trackEvent("tryon_started",{product_id:this.currentProduct?.id});try{let t=new FormData;t.append("user_photo",e),t.append("product_image_url",this.currentProduct?.imageUrl||""),t.append("product_id",this.currentProduct?.id||""),t.append("product_name",this.currentProduct?.name||""),t.append("brand_id",this.brandId),t.append("source","ghost-layer"),t.append("provider",this.selectedProvider);let r=await fetch(`${h}/api/widget/try-on`,{method:"POST",body:t});if(!r.ok){let i=await r.json().catch(()=>({}));throw new Error(i.debug||i.error||`Server error: ${r.status}`)}let o=await r.json(),n=o.result_url||o.resultUrl;if(!n)throw new Error("No result image returned");this.trackEvent("tryon_completed",{product_id:this.currentProduct?.id,result_url:n}),this.renderOverlay("result",{resultUrl:n})}catch(t){console.error("[GhostLayer] Try-on failed:",t),this.trackEvent("tryon_failed",{product_id:this.currentProduct?.id,error:String(t)}),this.renderOverlay("error",{errorMsg:t instanceof Error?t.message:"Please try again."})}}async trackEvent(e,t){try{await fetch(`${h}/api/widget/track`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({brand_id:this.brandId,event_name:e,event_data:t,page_url:location.href,timestamp:new Date().toISOString()}),keepalive:!0})}catch{}}};(function(){let e=(document.currentScript||document.querySelector('script[src*="ghostlayer-widget"]'))?.getAttribute("data-brand-id");if(!e){console.warn("[GhostLayer] No data-brand-id found on script tag");return}let t=()=>setTimeout(()=>{let r=new b(e);window.__ghostlayer=r},0);document.readyState==="loading"?document.addEventListener("DOMContentLoaded",t):t()})();})();
