...
<div class="gl-countdown-overlay" id="gl-countdown-overlay" style="display:none">
    ...
    `}bindOverlayEvents(e){let t=this.overlayShadow;if(t){if(t.getElementById("gl-close")?.addEventListener("click",()=>this.closeOverlay()),t.getElementById("gl-overlay")?.addEventListener("click",o=>{o.target.id==="gl-overlay"&&this.closeOverlay()}),e==="upload"){let o=t.getElementById("gl-generate-btn"),r=l=>{this.selectedFile=l,o&&(o.disabled=!1)},n=t.getElementById("gl-tab-upload"),i=t.getElementById("gl-tab-camera"),a=t.getElementById("gl-panel-upload"),s=t.getElementById("gl-panel-camera");n?.addEventListener("click",()=>{this.stopCamera(),n.classList.add("gl-tab-active"),i?.classList.remove("gl-tab-active"),a&&(a.style.display="block"),s&&(s.style.display="none")}),i?.addEventListener("click",async()=>{i.classList.add("gl-tab-active"),n?.classList.remove("gl-tab-active"),a&&(a.style.display="none"),s&&(s.style.display="block"),await this.startCamera(t)});let d=t.getElementById("gl-upload-zone"),u=t.getElementById("gl-file-input"),m=t.getElementById("gl-upload-preview-wrap"),c=t.getElementById("gl-upload-preview-img"),g=l=>{if(!l.type.startsWith("image/"))return;r(l);let p=URL.createObjectURL(l);c&&(c.src=p),d&&(d.style.display="none"),m&&(m.style.display="block")};d?.addEventListener("click",()=>u?.click()),t.getElementById("gl-change-photo")?.addEventListener("click",()=>{this.selectedFile=null,o&&(o.disabled=!0),d&&(d.style.display="block"),m&&(m.style.display="none"),u?.click()}),u?.addEventListener("change",()=>{let l=u.files?.[0];l&&g(l)}),d?.addEventListener("dragover",l=>{l.preventDefault(),d.classList.add("gl-drag-over")}),d?.addEventListener("dragleave",()=>d.classList.remove("gl-drag-over")),d?.addEventListener("drop",l=>{l.preventDefault(),d.classList.remove("gl-drag-over");let p=l.dataTransfer?.files[0];p&&g(p)}),t.getElementById("gl-capture-now")?.addEventListener("click",()=>{this.capturePhoto(t,r)}),t.getElementById("gl-timer-btn")?.addEventListener("click",()=>{this.startCountdown(t,5,r)}),t.getElementById("gl-retake-btn")?.addEventListener("click",async()=>{this.selectedFile=null,o&&(o.disabled=!0);let l=t.getElementById("gl-camera-preview-wrap"),p=t.getElementById("gl-camera-wrap"),h=t.getElementById("gl-camera-controls"),f=t.getElementById("gl-capture-now"),w=t.getElementById("gl-timer-btn"),E=t.getElementById("gl-lighting-bar");l&&(l.style.display="none"),p&&(p.style.display="block"),h&&(h.style.display="flex"),E&&(E.style.display="flex"),f&&(f.disabled=!0),w&&(w.disabled=!0),await this.startCamera(t)}),o?.addEventListener("click",async()=>{this.selectedFile&&await this.generateTryOn(this.selectedFile)});let b={primary:{512:.04,1024:.04,2048:.04,4096:.04},"gemini-2.5-flash-image":{512:.044,1024:.068,2048:.1,4096:.152},"gemini-3.1-flash-image-preview":{512:.09,1024:.134,2048:.202,4096:.302},"gemini-3.1-pro-image-preview":{512:.268,1024:.268,2048:.268,4096:.48},"gemini-3-pro-image-preview":{512:.268,1024:.268,2048:.268,4096:.48}},y=()=>{let l=t.getElementById("gl-cost-est");if(!l)return;let p=this.selectedProvider==="primary"?"primary":this.selectedGeminiModel,h=b[p]?.[this.selectedResolution]??.04;l.textContent=`$${h.toFixed(3)}`};t.querySelectorAll(".gl-model-card").forEach(l=>{l.addEventListener("click",()=>{t.querySelectorAll(".gl-model-card").forEach(f=>f.classList.remove("gl-model-active")),l.classList.add("gl-model-active");let p=l.dataset.provider,h=l.dataset.model??"";this.selectedProvider=p,this.selectedGeminiModel=h,y()})}),t.querySelectorAll(".gl-res-btn").forEach(l=>{l.addEventListener("click",()=>{t.querySelectorAll(".gl-res-btn").forEach(p=>p.classList.remove("gl-res-active")),l.classList.add("gl-res-active"),this.selectedResolution=l.dataset.res??"512",y()})}),y(),t.getElementById("gl-buy-btn-upload")?.addEventListener("click",()=>{let l=this.currentProduct?.id;this.trackEvent("buy_clicked",{product_id:l}),l&&window.glAddToCart?window.glAddToCart(l):(document.querySelector('button[name="add"]')||document.querySelector('[class*="add-to-cart"]'))?.click(),this.closeOverlay()}),t.getElementById("gl-wl-btn-upload")?.addEventListener("click",()=>{let l=this.currentProduct?.id;if(this.trackEvent("wishlist_clicked",{product_id:l}),l&&window.glAddToWL){window.glAddToWL(l);let p=t.getElementById("gl-wl-btn-upload");p&&(p.textContent="\u2665 Wishlisted")}})}e==="result"&&(t.getElementById("gl-retry")?.addEventListener("click",()=>{this.selectedFile=null,this.renderOverlay("upload")}),t.getElementById("gl-buy-btn")?.addEventListener("click",()=>{let o=this.currentProduct?.id;this.trackEvent("buy_clicked",{product_id:o}),o&&window.glAddToCart?window.glAddToCart(o):(document.querySelector('button[name="add"]')||document.querySelector('[class*="add-to-cart"]'))?.click(),this.closeOverlay()}),t.getElementById("gl-wl-btn")?.addEventListener("click",()=>{let o=this.currentProduct?.id;if(this.trackEvent("wishlist_clicked",{product_id:o}),o&&window.glAddToWL){window.glAddToWL(o);let r=t.getElementById("gl-wl-btn");r&&(r.textContent="\u2665 Wishlisted")}})),e==="error"&&t.getElementById("gl-retry")?.addEventListener("click",()=>{this.selectedFile=null,this.renderOverlay("upload")})}}async startCamera(e){let t=e.getElementById("gl-camera-video"),o=e.getElementById("gl-camera-error"),r=e.getElementById("gl-capture-now"),n=e.getElementById("gl-timer-btn"),i=e.getElementById("gl-camera-wrap");this.stopCamera();try{let a=await navigator.mediaDevices.getUserMedia({video:{facingMode:"user",width:{ideal:640},height:{ideal:480}}});this.cameraStream=a,t&&(t.srcObject=a),i&&(i.style.display="block"),o&&(o.style.display="none"),this.monitorLighting(e)}catch{o&&(o.style.display="block"),i&&(i.style.display="none"),r&&(r.disabled=!0),n&&(n.disabled=!0)}}capturePhoto(e,t){let o=e.getElementById("gl-camera-video"),r=e.getElementById("gl-camera-canvas");if(!o||!r||!o.videoWidth)return;r.width=o.videoWidth,r.height=o.videoHeight;let n=r.getContext("2d");if(n){if(this.lastBrightness>0&&this.lastBrightness<80){let i=Math.min(2,100/Math.max(this.lastBrightness,20));n.filter=`brightness(${i.toFixed(2)})`}n.drawImage(o,0,0),n.filter="none",this.stopCamera(),r.toBlob(i=>{if(!i)return;let a=new File([i],"selfie.jpg",{type:"image/jpeg"}),s=e.getElementById("gl-camera-wrap"),d=e.getElementById("gl-camera-controls"),u=e.getElementById("gl-lighting-bar"),m=e.getElementById("gl-camera-preview-wrap"),c=e.getElementById("gl-camera-preview-img"),g=URL.createObjectURL(a);c&&(c.src=g),s&&(s.style.display="none"),d&&(d.style.display="none"),u&&(u.style.display="none"),m&&(m.style.display="block"),t(a)},"image/jpeg",.92)}}monitorLighting(e){this.lightingInterval&&(clearInterval(this.lightingInterval),this.lightingInterval=null);let t=e.getElementById("gl-camera-video"),o=e.getElementById("gl-lighting-dot"),r=e.getElementById("gl-lighting-text"),n=e.getElementById("gl-capture-now"),i=e.getElementById("gl-timer-btn"),a=document.createElement("canvas");a.width=64,a.height=48;let s=a.getContext("2d");this.lightingInterval=window.setInterval(()=>{if(!s||!t||!t.videoWidth)return;s.drawImage(t,0,0,64,48);let d=s.getImageData(0,0,64,48).data,u=0;for(let b=0;b<d.length;b+=4)u+=.299*d[b]+.587*d[b+1]+.114*d[b+2];let m=u/(64*48);this.lastBrightness=m;let c=m>=80,g=m>=50;o&&(o.style.background=c?"#22c55e":g?"#f59e0b":"#ef4444"),r&&(r.textContent=c?"\u2713 Good lighting":g?"\u26A0 Brighter light will improve results":"\u2717 Too dark \u2014 move to better lighting"),this.countdownTimer||(n&&(n.disabled=!g),i&&(i.disabled=!g))},500)}startCountdown(e,t,o){let r=e.getElementById("gl-countdown-overlay"),n=e.getElementById("gl-countdown-num"),i=e.getElementById("gl-capture-now"),a=e.getElementById("gl-timer-btn");this.countdownTimer&&(clearInterval(this.countdownTimer),this.countdownTimer=null),r&&(r.style.display="flex"),i&&(i.disabled=!0),a&&(a.disabled=!0);let s=t;n&&(n.textContent=String(s)),this.countdownTimer=window.setInterval(()=>{s--,n&&(n.textContent=String(s)),s<=0&&(clearInterval(this.countdownTimer),this.countdownTimer=null,r&&(r.style.display="none"),i&&(i.disabled=!1),a&&(a.disabled=!1),this.capturePhoto(e,o))},1e3)}stopCamera(){this.countdownTimer&&(clearInterval(this.countdownTimer),this.countdownTimer=null),this.lightingInterval&&(clearInterval(this.lightingInterval),this.lightingInterval=null),this.cameraStream&&(this.cameraStream.getTracks().forEach(e=>e.stop()),this.cameraStream=null)}closeOverlay(){this.stopCamera(),this.overlayShadow?.getElementById("gl-overlay")?.remove(),this.trackEvent("tryon_closed",{product_id:this.currentProduct?.id})}tryOnCacheKey(e){return`gl_tryon_${this.currentProduct?.id}_${e.name}_${e.size}`}async generateTryOn(e){this.stopCamera();let t=this.tryOnCacheKey(e),o=sessionStorage.getItem(t);if(o){this.trackEvent("tryon_completed",{product_id:this.currentProduct?.id,result_url:o,cached:!0}),this.renderOverlay("result",{resultUrl:o});return}this.renderOverlay("processing"),this.trackEvent("tryon_started",{product_id:this.currentProduct?.id});try{let r=new FormData;r.append("user_photo",e),r.append("product_image_url",this.currentProduct?.imageUrl||""),r.append("product_id",this.currentProduct?.id||""),r.append("product_name",this.currentProduct?.name||""),r.append("brand_id",this.brandId),r.append("source","ghost-layer"),r.append("provider",this.selectedProvider),this.selectedProvider==="fallback"&&(r.append("gemini_model",this.selectedGeminiModel),r.append("output_resolution",this.selectedResolution));let n=await fetch(`${v}/api/widget/try-on`,{method:"POST",body:r});if(!n.ok){let s=await n.json().catch(()=>({}));throw new Error(s.debug||s.error||`Server error: ${n.status}`)}let i=await n.json(),a=i.result_url||i.resultUrl;if(!a)throw new Error("No result image returned");try{sessionStorage.setItem(t,a)}catch{}this.trackEvent("tryon_completed",{product_id:this.currentProduct?.id,result_url:a}),this.renderOverlay("result",{resultUrl:a})}catch(r){console.error("[GhostLayer] Try-on failed:",r),this.trackEvent("tryon_failed",{product_id:this.currentProduct?.id,error:String(r)}),this.renderOverlay("error",{errorMsg:r instanceof Error?r.message:"Please try again."})}}async trackEvent(e,t){try{await fetch(`${v}/api/widget/track`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({brand_id:this.brandId,event_name:e,event_data:t,page_url:location.href,timestamp:new Date().toISOString()}),keepalive:!0})}catch{}}};(function(){let e=(document.currentScript||document.querySelector('script[src*="ghostlayer-widget"]'))?.getAttribute("data-brand-id");if(!e){console.warn("[GhostLayer] No data-brand-id found on script tag");return}let t=()=>setTimeout(()=>{let o=new x(e);window.__ghostlayer=o},0);document.readyState==="loading"?document.addEventListener("DOMContentLoaded",t):t()})();})();
