...
MODESTY (conditional — trigger: outfit in IMAGE 1 would expose the midriff): If — and ONLY if — the outfit in IMAGE 1 would otherwise leave the customer's stomach, midriff, or navel exposed on her body (e.g. cropped choli + low-rise lehenga with a waist gap, crop top + jeans, short blouse + skirt, sleeveless midriff-baring cut), close that gap on the customer by extending the TOP piece downward using its own fabric, color, and pattern so the midriff is fully covered. If the outfit already covers the midriff (long kameez, maxi, full-length gown, long shirt + trousers, abaya, full kurta, etc.), the trigger is NOT present — do nothing extra, render the outfit exactly as shown in IMAGE 1 without altering its design or length.

QUALITY UPLIFT (conditional — trigger: IMAGE 2 is visibly blurry/soft/low-resolution): If — and ONLY if — IMAGE 2 is noticeably blurry, soft, out-of-focus, or low-resolution, you MAY improve the OUTPUT's clarity (sharpen edges, reduce noise, recover detail) at the level of image quality only. This is NOT permission to alter the person's face, features, expression, skin tone, body, or identity — the IDENTITY LOCK still fully binds. If IMAGE 2 is already sharp and well-exposed, the trigger is NOT present — do NOT sharpen, smooth, or apply any quality changes; render the customer at her original quality.` }],
    },
    contents: [
      {
        role: 'user',
